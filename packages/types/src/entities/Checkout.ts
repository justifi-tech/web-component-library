...
// Extracted from ModularCheckout.ts for shared use
...
export class Checkout implements ICheckout {
  ...
  get payment_mode(): ICheckoutPaymentMode {
    let paymentMode: ICheckoutPaymentMode = ICheckoutPaymentMode.unknown;
    ...
    } else if ((lastCompletion.payment_response as { data?: { payment_method?: { card?: unknown } } })?.data?.payment_method?.card) {
      paymentMode = ICheckoutPaymentMode.card;
    } else if (
      (lastCompletion.payment_response as { data?: { payment_method?: { bank_account?: unknown } } })?.data?.payment_method?.bank_account
    ) {
