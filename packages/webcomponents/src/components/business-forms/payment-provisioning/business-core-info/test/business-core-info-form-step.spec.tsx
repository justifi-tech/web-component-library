...
describe.skip('business-core-info-form-step', () => {
  let consoleSpy;
  ...
  it('should log a warning if no authToken is provided', async () => {
    await newSpecPage({
      ...
      html: `<business-core-info-form-step></business-core-info-form-step>`,
    });
  ...
  it('should emit server error event if server error when fetching data', async () => {
    await newSpecPage({
      components: [BusinessCoreInfoFormStep],
      html: `<business-core-info-form-step business-id="biz_123" auth-token="some-token"></business-core-info-form-step>`,
    });
