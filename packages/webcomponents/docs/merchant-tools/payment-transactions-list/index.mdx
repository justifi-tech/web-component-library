...
<UsageSnippet
  title="Listening for click events"
  code={`const transactionsList = document.querySelector('justifi-payment-transactions-list');

transactionsList.addEventListener('click-event', (event) => {
console.log('Transaction clicked', event.detail);
});`}
language="js"
/>
