...
`<justifi-season-interruption-insurance>` fetches an insurance quote for the active checkout and renders an accept/decline pair of radio inputs. It uses the same identity and policy data the checkout already collected and fires updates whenever the customer changes their selection.
