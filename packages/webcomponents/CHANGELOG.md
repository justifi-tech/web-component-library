...
- ccd3d05: Add `useNativePaymentRequest` prop to `justifi-google-pay` for Android WebView embeds; routes Google Pay through the W3C Payment Request API instead of pay.js's `loadPaymentData` popup (blocked in WebViews, OR_BIBED_15).
...
- 19eb390: Add missing payout transaction type labels for fee balance transactions (processing_fee, refund_processing_fee, fee_rounding_adjustment, platform_fee, processing_fee_return, platform_fee_return)
...
- bd81b03: fix: prevent double submission in checkout via checkoutStore
