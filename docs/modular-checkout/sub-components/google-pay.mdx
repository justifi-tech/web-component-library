...
1. Component initializes iframe with auth credentials
2. Iframe loads Google Pay SDK and checks device eligibility
3. If eligible, Google Pay button is rendered
4. User clicks button → payment sheet opens
5. User completes payment → iframe processes token with JustiFi API
6. `googlePayCompleted` event fires with `paymentMethodId`
7. Modular checkout automatically completes the checkout
