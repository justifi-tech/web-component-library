...
For pre-release versions, set the `@justifi/webcomponents` version in `packages/webcomponents/package.json` to an `-rc.N` suffix (e.g. `6.15.0-rc.1`) and merge that bump to `main`. The **Publish packages** workflow (`publish.yml`) detects the `-rc.N` suffix and publishes to the `next` npm tag via `npm publish --tag next` instead of running `changeset publish`. Consumers install RCs with `@justifi/webcomponents@next`.
