...
Migration notes:
- Errors - instead of getting errors via `data.errors[0]`, which was an error code, you will now need to do `data.error.code`. There is also now a human readable message (`data.error.message`)
