...
<Meta title="Payment Facilitation/Merchant Tools/Payment Details" Components="justifi-payment-details" />
...
<ExportedParts
  ...
  tags={filterDocsByTag('justifi-payment-details').docsTags.map(
    (part) => part.text
  )}
/>
