...
> **Note:** If you are using the card payment method, you also need to provide billing information. This can be done by using either the `justifi-billing-form-full` component for complete billing address, the `justifi-card-billing-form-simple` component for ZIP code only, or by passing the `billingInfo` object as an argument to the `submitCheckout` method called on the wrapper. The `billingInfo` object can contain all the fields or just the `address_postal_code` field.
