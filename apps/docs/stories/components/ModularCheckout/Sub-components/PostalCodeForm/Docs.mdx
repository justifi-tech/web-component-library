import { Meta, Title, ArgTypes, Source } from '@storybook/blocks';
import { filterDocsByTag, ExportedParts } from '../../../../utils';
import { codeExampleFull } from './example';
import { ComponentBox } from '../../../../utils/documentation-components';
import * as JustifiPostalCodeForm from './index.stories';
import { setUpMocks } from '../../../../utils/mockAllServices';

{setUpMocks()}

<Meta
  title="Modular Checkout/Sub-components/Postal Code Form"
  Components="justifi-postal-code-form"
  of={JustifiPostalCodeForm}
/>

<Title />

---

Renders a form for collecting the postal code as part of a checkout flow.

This component is **designed to be used within** the `justifi-modular-checkout` and **does not accept props directly**. Instead, it relies on the shared state passed through the Stencil Store, managed by the `justifi-modular-checkout` component.

> **Note:** This component is specifically designed to be used in conjunction with the `justifi-card-form` component. Alternatively, you can provide the postal code by passing a `billingInformation` object with an `address_postal_code` field as an argument to the `submitCheckout` method called on the `justifi-modular-checkout` component. See the [Modular Checkout docs](/docs/modular-checkout-modular-checkout--docs) for more information.

**Authorization** and business context are also handled by `justifi-modular-checkout`, which manages authentication tokens and related configuration.

This component exposes **no public methods or properties** and is not intended for standalone use.

# Example Usage

---

<ComponentBox>
  <justifi-modular-checkout
    account-id="acc_123"
    auth-token="123abc"
    checkout-id="cho_123"
  >
    <justifi-card-form />
    <justifi-postal-code-form />
  </justifi-modular-checkout>
</ComponentBox>

---

<Source dark language="html" code={codeExampleFull} />
