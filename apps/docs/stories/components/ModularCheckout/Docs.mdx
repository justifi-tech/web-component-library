...
modularCheckout.setSelectedPaymentMethod({ type: 'card' });

// Set to new bank account payment method
modularCheckout.setSelectedPaymentMethod({ type: 'bankAccount' });

// Set to a saved payment method by id
modularCheckout.setSelectedPaymentMethod({ id: 'pm_123', type: 'card' });
```
...
<ArgTypes exclude={['Theme', 'slot', 'account-id']} />
