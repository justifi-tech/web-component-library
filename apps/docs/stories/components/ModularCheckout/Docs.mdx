...
The `justifi-modular-checkout` wrapper component emits a `checkout-changed` event whenever any internal checkout state changes. The event detail includes the currently computed `availablePaymentMethods`.

```javascript
const modularCheckout = document.querySelector('justifi-modular-checkout');
modularCheckout.addEventListener('checkout-changed', (event) => {
  const { availablePaymentMethods } = event.detail;
