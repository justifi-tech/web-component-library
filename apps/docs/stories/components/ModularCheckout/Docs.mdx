...
- [Reading Available Payment Methods](#reading-available-payment-methods)
...
## Reading Available Payment Methods

You can retrieve the list of currently available payment methods based on the checkout's `payment_settings` and component flags using the `getAvailablePaymentMethods` method. This returns a promise that resolves to an array of method IDs.

```javascript
const modularCheckout = document.querySelector('justifi-modular-checkout');
const available = await modularCheckout.getAvailablePaymentMethods();
// Example result: ['saved_card', 'new_card', 'new_bank_account', 'sezzle']
```

The returned values map to:
