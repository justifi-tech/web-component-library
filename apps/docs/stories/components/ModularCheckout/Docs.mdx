...
- [Pre-Submit Hook](#pre-submit-hook)
...
You can provide a `preSubmitHook` function to inspect the checkout state before submission proceeds. This is useful for implementing custom validation, user confirmation dialogs, or conditional payment restrictions based on business rules.

The hook receives three parameters:
- `state`: A `CheckoutState` object containing the current checkout information
- `next`: A function to call to proceed with submission
- `cancel`: A function to call to stop submission

```javascript
const checkout = document.querySelector('justifi-modular-checkout');

checkout.preSubmitHook = (state, next, cancel) => {
...
The `CheckoutState` object includes:
- `selectedPaymentMethod`: Currently selected payment method
- `paymentAmount`: Payment amount in cents
- `totalAmount`: Total amount including fees in cents
- `paymentCurrency`: Currency code (e.g., 'USD')
- `paymentDescription`: Payment description
- `savedPaymentMethods`: Array of saved payment methods
- `savePaymentMethod`: Whether to save the payment method
- Payment method availability flags (`bnplEnabled`, `applePayEnabled`, etc.)
