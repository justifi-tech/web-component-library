...
- [justifi-google-pay](/docs/modular-checkout-google-pay--docs)
...
- `sezzle`: BNPL is available when enabled on the account and not disabled.
- `plaid`: Plaid is available when bank account verification is enabled for the checkout.
- `apple_pay`: Apple Pay is available when enabled on the account and not disabled.
- `google_pay`: Google Pay is available when enabled on the account and not disabled.
