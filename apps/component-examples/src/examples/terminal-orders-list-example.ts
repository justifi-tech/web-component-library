...
import {
  TerminalOrdersListComponent,
  TerminalOrdersListData,
} from '../components/TerminalOrdersListComponent';

// Load environment variables
require('dotenv').config({ path: '../../.env' });

// Create the JavaScript for the terminal orders list component
function createTerminalOrdersListScript(): string {
  return `
...
// Main terminal orders list example handler
async function handleTerminalOrdersListExample(
  req: any,
  res: any
): Promise<void> {
  try {
    ...
    const componentData: TerminalOrdersListData = {
      subAccountId,
      webComponentToken,
    };
    ...
    const bodyContent = renderToString(
      TerminalOrdersListComponent(componentData)
    );
    const script = createTerminalOrdersListScript();
    ...
    console.error('Error in terminal orders list example:', error);
...
// Add the terminal orders list route
server.addRoute({
  path: '/',
  handler: handleTerminalOrdersListExample,
});
