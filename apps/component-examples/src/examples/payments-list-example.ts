...
import {
  ...
  PaymentsListData,
} from '../components/PaymentsListComponent';

// Load environment variables
require('dotenv').config({ path: '../../.env' });

// Create the JavaScript for the payments list component
function createPaymentsListScript(): string {
  return `
...
// Main payments list example handler
async function handlePaymentsListExample(req: any, res: any): Promise<void> {
  try {
    ...
    const componentData: PaymentsListData = {
      subAccountId,
      webComponentToken,
    };

    // Render the template using JSX and convert to HTML string
    const bodyContent = renderToString(PaymentsListComponent(componentData));
    const script = createPaymentsListScript();
    ...
    console.error('Error in payments list example:', error);
...
// Add the payments list route
server.addRoute({
  path: '/',
  handler: handlePaymentsListExample,
});
